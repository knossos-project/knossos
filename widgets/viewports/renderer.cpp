...
//    }
    ...
    triangleVertices.push_back({{gpucubeshape.z, 0.0f, 0.0f}});
    triangleVertices.push_back({{gpucubeshape.z, gpucubeshape.y, 0.0f}});
    ...
    for (float x = 0.0f; x < (zy ? 1 : gpusupercube.x); ++x) {
        const float frame = std::fmod(xy ? cpos.z : xz ? cpos.y : cpos.x, xy ? gpucubeshape.z : xz ? gpucubeshape.y : gpucubeshape.x);
        const auto texR = (0.5f + frame) / (xy ? gpucubeshape.y : xz ? gpucubeshape.z : gpucubeshape.x);
    ...
    if (layer.enabled && layer.opacity >= 0.0f && !(state->viewerState->showOnlyRawData && layer.isOverlayData)) {
        ...
        if (!arb) {
            const auto halfsc = (v1.componentMul(fov) * 0.5) / gpucubeshape;
            const auto offset = cpos / gpucubeshape - halfsc.componentMul(!zy,!xz,!xy);
            const auto start = (cpos * 0) / gpucubeshape;
            // floatCoordinate(zy,xz,-xy) == vp.n
            const auto end = start + floatCoordinate(zy,xz,xy) + gpusupercube.componentMul(!zy,!xz,!xy);
            for (float z = start.z; z < end.z; ++z)
            for (float y = start.y; y < end.y; ++y)
            for (float x = start.x; x < end.x; ++x) {
                const auto gcubepos = CoordOfGPUCube(offset.x + x, offset.y + y, offset.z + z);
            ...
            for (auto & pair : layer.textures) {
                ...
                if (!cube.vertices.empty()) {
                    ...
                    for (const auto & vertex : cube.vertices) {
                        ...
                        const auto depthOffset = static_cast<float>(vertex.z - pos.z * gpucubeshape.x);
                        const auto texR = (0.5f + depthOffset) / gpucubeshape.x;
                        textureVertices.push_back({{static_cast<float>(vertex.x - pos.x * gpucubeshape.x) / gpucubeshape.x
                                                    , static_cast<float>(vertex.y - pos.y * gpucubeshape.x) / gpucubeshape.x
                                                    , texR}});
                    }
                    render(cube);
                }
