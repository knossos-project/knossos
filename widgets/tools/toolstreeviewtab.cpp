    ...
    , radiusBuffer(state->skeletonState->defaultNodeRadius), draggedNodeID(0), displayedNodes(1000)
{
    ...
    QObject::connect(treeTable, &TreeTable::updateNodesTable, this, &ToolsTreeviewTab::updateNodesTable);
...
void ToolsTreeviewTab::treeContextMenuCalled(QPoint pos) {
    treeContextMenu->actions().at(0)->setEnabled(focusedTreeTable != activeTreeTable && state->skeletonState->selectedTrees.size() == 1); // set as active tree action
...
void ToolsTreeviewTab::updateTreeCommentBuffer(const QString & comment) {
...
void ToolsTreeviewTab::editTreeColor() {
    if (state->skeletonState->selectedTrees.size() != 1) {
        qDebug("Failed to edit tree color. Not exactly one tree selected.");
    } else {
        state->skeletonState->selectedTrees.front()->color.r = rSpin->value();
        state->skeletonState->selectedTrees.front()->color.g = gSpin->value();
        state->skeletonState->selectedTrees.front()->color.b = bSpin->value();
        state->skeletonState->selectedTrees.front()->color.a = aSpin->value();
...
void ToolsTreeviewTab::moveNodesAction() {
    if(state->skeletonState->selectedNodes.size() == 0 || state->skeletonState->selectedTrees.size() != 1) {
        QMessageBox prompt;
        prompt.setWindowFlags(Qt::WindowStaysOnTopHint);
        prompt.setIcon(QMessageBox::Information);
        prompt.setWindowTitle("Information");
...
void ToolsTreeviewTab::nodeItemDoubleClicked(QTableWidgetItem*) {
    ...
    emit setActiveNodeSignal(CHANGE_MANUAL, nullptr, state->skeletonState->selectedNodes.at(0)->nodeID);
...
void ToolsTreeviewTab::treesDeleted() {
    if(focusedTreeTable == activeTreeTable) {
        ...
        if(activeTreeTable->item(0, TREE_ID) != nullptr) {
            if(state->skeletonState->activeTree == nullptr) {
                activeTreeTable->clearContents();
            }
    ...
    if(activeNodeTable->item(0, NODE_ID) != nullptr) {
    ...
    for(int i = 0; i < nodeRows; ++i) {
        if(Skeletonizer::findNodeByNodeID(nodeTable->item(i, NODE_ID)->text().toInt()) == nullptr) {
...
void ToolsTreeviewTab::nodeCommentChanged(nodeListElement *node) {
    if(node == state->skeletonState->activeNode) {
        ...
        for(int i = 0; i < nodeTable->rowCount(); ++i) {
            if(nodeTable->item(i, NODE_ID)->text().toUInt() == node->nodeID) {
                if(node->comment) {
...
int ToolsTreeviewTab::getActiveNodeRow() {
    int activeRow = -1;
    for(int i = 0; i < nodeTable->rowCount(); ++i) {
        if(nodeTable->item(i, NODE_ID)->text().toUInt() == state->skeletonState->activeNode->nodeID) {
            activeRow = i;
            break;
        }
    }
