...
static int32_t clientRun() {
    ...
    message = malloc(8192 * sizeof(Byte));
    if(message == NULL) {
        LOG("Out of memory");
        _Exit(FALSE);
    }
    ...
    if(connectToServer() == TRUE) {
        ...
        message[0] = KIKI_HI;
        integerToBytes(&message[1], messageLen);
        strncpy((char *)&message[5], state->name, nameLen);
        floatToBytes(&message[5 + nameLen], state->scale.x);
        floatToBytes(&message[5 + nameLen + 4], state->scale.y);
        floatToBytes(&message[5 + nameLen + 8], state->scale.z);
        integerToBytes(&message[5 + nameLen + 12], state->offset.x);
        ...
        while(TRUE) {
             ...
             * 100 ms timeout.
             */
            SDLNet_CheckSockets(clientState->socketSet, 100);

            if(SDLNet_SocketReady(clientState->remoteSocket)) {
                readLen = SDLNet_TCP_Recv(clientState->remoteSocket, message, 8192);
                if(readLen == 0) {
...
static int32_t connectToServer() {
    ...
    SDL_Thread *ThreadWrapper_SDLNet_TCP_Open;
    ...
    while(clientState->inBuffer->length >= 5) {
        switch(clientState->inBuffer->data[0]) {
            ...
            case KIKI_ADDNODE:
                ...
                pPosition = malloc(sizeof(Coordinate));
                if(pPosition == NULL) {
                    LOG("Out of memory.");
                    goto critical;
                }
        ...
        switch(*fmt++) {
            ...
            case 'd':
                if(++dIndex >= INTS || (pos += 4) > len)
                    goto overflow;
                d[dIndex] = bytesToInt(&buffer->data[pos - 4]);
                break;
            case 'f':
...
static uint32_t flushOutBuffer() {
    ...
    SDL_UnlockMutex(state->protectOutBuffer);
...
uint32_t IOBufferAppend(
    ...
    if(length > (iobuffer->size - iobuffer->length)) {
        ...
        if(iobuffer->size < MAX_BUFFER_SIZE) {
            ...
            newDataPtr = realloc(iobuffer->data, newSize);
            if(newDataPtr == NULL) {
...
uint32_t addPeer(
    ...
    newPeer = malloc(sizeof(struct peerListElement));
    if(newPeer == NULL) {
...
Coordinate *transNetCoordinate(
    ...
    struct clientState *clientState = state->clientState;
    struct peerListElement *peer = clientState->firstPeer;
    ...
    outCoordinate = malloc(sizeof(Coordinate));
...
uint32_t broadcastPosition(
    ...
    data[0] = KIKI_REPEAT;
    integerToBytes(&data[1], messageLen);
    data[5] = KIKI_POSITION;
    integerToBytes(&data[6], clientState->myId);
    integerToBytes(&data[10], x);
    ...
    while(*fmt) {
        switch(*fmt++) {
            case 'd':
                ...
                integerToBytes(&packedBytes[len - 4], d);
                break;
            ...
            case 'r':
                ...
                integerToBytes(&packedBytes[len - 4], state->skeletonState->skeletonRevision);
