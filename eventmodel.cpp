    ...
    if(Patch::patchMode && VPfound < VIEWPORT_SKELETON) {
        ...
        if(Patch::drawMode == DRAW_CONTINUOUS_LINE) {
            ...
            if(Patch::eraseInVP == VPfound) { // erasing
                ...
                for(uint i = 0; i < Patch::lineBuffer.size(); ++i) { // test if line can be aligned to existing line
                    switch(VPfound) {
                    case VIEWPORT_XY:
                        if(Patch::lineBuffer[i][0].z != clickedCoordinate->z) {
                            continue;
                        }
                        break;
                    case VIEWPORT_XZ:
                        if(Patch::lineBuffer[i][0].y != clickedCoordinate->y) {
                            continue;
                        }
                        break;
                    case VIEWPORT_YZ:
                        if(Patch::lineBuffer[i][0].x != clickedCoordinate->x) {
                            continue;
                        }
                        break;
                    }
...
bool EventModel::handleMouseReleaseRight(QMouseEvent *event, int VPfound) {
    if(Patch::patchMode) {
        Patch::drawing = false;
        if(Patch::newPoints) {
...
bool EventModel::handleMouseReleaseMiddle(QMouseEvent *event, int VPfound) {    // a node was dragged to a new position
    if(state->skeletonState->activeNode) {
...
bool EventModel::handleKeyPress(QKeyEvent *event, int VPfound) {
    ...
    if(event->key() == Qt::Key_Left) {
        ...
        if(shift) {
            ...
            case VIEWPORT_ARBITRARY:
                emit userMoveArbSignal(
                    state->viewerState->vpConfigs[VPfound].n.x * 10.0 * (float)state->magnification,
                    state->viewerState->vpConfigs[VPfound].n.y * 10.0 * (float)state->magnification,
                    state->viewerState->vpConfigs[VPfound].n.z * 10.0 * (float)state->magnification,
                    TELL_COORDINATE_CHANGE);
                break;
            }
        } else {
            ...
            case VIEWPORT_XY:
                state->newCoord[0] = 0;
                state->newCoord[1] = 0;
                state->newCoord[2] = state->viewerState->vpKeyDirection[VIEWPORT_XY] * state->viewerState->dropFrames
                                     * state->magnification;
                break;
            case VIEWPORT_XZ:
                state->newCoord[0] = 0;
                state->newCoord[1] = state->viewerState->vpKeyDirection[VIEWPORT_XZ] * state->viewerState->dropFrames
                                     * state->magnification;
                state->newCoord[2] = 0;
                break;
            case VIEWPORT_YZ:
                state->newCoord[0] = state->viewerState->vpKeyDirection[VIEWPORT_YZ] * state->viewerState->dropFrames
                                     * state->magnification;
                state->newCoord[1] = 0;
                state->newCoord[2] = 0;
                break;
            case VIEWPORT_ARBITRARY:
                emit userMoveArbSignal(state->viewerState->vpConfigs[VPfound].n.x
                                       * (float)state->viewerState->dropFrames * (float)state->magnification,
                                state->viewerState->vpConfigs[VPfound].n.y * (float)state->viewerState->dropFrames
                                       * (float)state->magnification,
                                state->viewerState->vpConfigs[VPfound].n.z * (float)state->viewerState->dropFrames
                                       * (float)state->magnification,
                                TELL_COORDINATE_CHANGE);
                 break;
            }
        ...
        if(shift) {
            ...
            case VIEWPORT_ARBITRARY:
                emit userMoveArbSignal(
                    state->viewerState->vpConfigs[VPfound].n.x
                        * (float)state->viewerState->vpKeyDirection[VPfound]
                        * -10.0 * (float)state->magnification,
                    state->newCoord[1] = state->viewerState->vpConfigs[VPfound].n.y
                        * (float)state->viewerState->vpKeyDirection[VPfound]
                        * -10.0 * (float)state->magnification,
                    state->newCoord[2] = state->viewerState->vpConfigs[VPfound].n.z
                        * (float)state->viewerState->vpKeyDirection[VPfound]
                        * -10.0 * (float)state->magnification,
                    TELL_COORDINATE_CHANGE);
                break;
        ...
        if(control) {
            if(Patch::patchMode) {
                if(newPatchSignal()) {
                    Patch::genRandCloud(100000);
                    //Patch::genRandTriangulation(1000);
                    emit updatePatchesWidget();
                }
            }
        }
        else if(Patch::patchMode) {
//            qDebug("start triangulation");
//            Patch::activePatch->computeTriangles();
//            qDebug("finished triangulation");
//            emit updatePatchesWidget();
            if(Patch::activePatch) {
                Patch::visiblePoints(VIEWPORT_XY);
            }
            //pcl::io::savePLYFile("meshing.ply", Patch::activePatch->mesh);
        }
...
floatCoordinate *EventModel::getFloatCoordinateFromOrthogonalClick(QMouseEvent *event, int VPfound) {
    floatCoordinate *foundCoordinate;
    foundCoordinate = static_cast<floatCoordinate*>(malloc(sizeof(floatCoordinate)));
    float x, y, z;
    x = y = z = 0;
    ...
    case VIEWPORT_ARBITRARY:
        /* @arb */
        x = state->viewerState->vpConfigs[VPfound].leftUpperDataPxOnScreen_float.x
        ...
        y = state->viewerState->vpConfigs[VPfound].leftUpperDataPxOnScreen_float.y
        ...
        z = state->viewerState->vpConfigs[VPfound].leftUpperDataPxOnScreen_float.z
