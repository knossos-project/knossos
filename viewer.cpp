    ...
    for(x_dc = 0; x_dc < currentVp->vpConfig->texture.usedTexLengthDc; x_dc++) {
        for(y_dc = 0; y_dc < currentVp->vpConfig->texture.usedTexLengthDc; y_dc++) {
            ...
            glBindTexture(GL_TEXTURE_2D,
                          currentVp->vpConfig->texture.dataTexHandle);
            ...
            if(state->overlay) {
                glBindTexture(GL_TEXTURE_2D,
                              currentVp->vpConfig->texture.dataOverlayHandle);
                glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
...
bool Viewer::vpGenerateTexture_arb(struct vpListElement *currentVp) {
    ...
    glBindTexture(GL_TEXTURE_2D, currentVp->vpConfig->texture.dataTexHandle);
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
...
bool Viewer::updateViewerState() {
   uint i;
    ...
    for(i = 0; i < state->viewerState->numberViewports; i++) {
        ...
        glBindTexture(GL_TEXTURE_2D, state->viewerState->vpConfigs[i].texture.dataOverlayHandle);
        // Set the parameters for the texture.
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, state->viewerState->filterType);
        glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, state->viewerState->filterType);
    }
...
//                    skeletonizer, SLOT(findNodeByNodeID(int)));
    ...
    // patch related signals
//    connect(eventModel, SIGNAL(updatePatchesWidget()),
...
//                    this, SLOT(updateActivePatchConnections()));
    connect(skeletonizer, SIGNAL(activePatchChanged()), this, SLOT(updateActivePatchConnections()));
//    connect(window->widgetContainer->toolsWidget->toolsPatchesTabWidget, SIGNAL(delActivePatchSignal()),
//                    skeletonizer, SLOT(delActivePatch()));
    // end patch related signals
}

void Viewer::updateActivePatchConnections() {
    connect(vpUpperLeft, SIGNAL(updateDistinguishableTriangles(int)),
                    Patch::activePatch, SLOT(updateDistinguishableTriangles(int)));
    connect(vpLowerLeft, SIGNAL(updateDistinguishableTriangles(int)),
                    Patch::activePatch, SLOT(updateDistinguishableTriangles(int)));
    connect(vpUpperRight, SIGNAL(updateDistinguishableTriangles(int)),
                    Patch::activePatch, SLOT(updateDistinguishableTriangles(int)));
