                ...
                if (cube != nullptr) {
                    if (Dataset::datasets[layerId].isOverlay()) {
                        ...
                        if (Dataset::datasets[layerId].bytesPerVoxel == 2) {
                            dcSliceExtract(reinterpret_cast<std::uint16_t *>(cube) + slicePositionWithinCube, slicePosInAbsPx, vp.texture.texData[layerId].data() + index, vp, layerId, adjustment, combine);
                        } else {
                            dcSliceExtract(reinterpret_cast<std::uint8_t  *>(cube) + slicePositionWithinCube, slicePosInAbsPx, vp.texture.texData[layerId].data() + index, vp, layerId, adjustment, combine);
                        }
                    }
                } else {
...
void Viewer::vpGenerateTexture(ViewportArb &vp, const std::size_t layerId) {
    ...
    while(s < vp.texture.usedSizeInCubePixels) {
        ...
        while(t < vp.texture.usedSizeInCubePixels) {
            ...
            // dispatch strictly on this layer’s depth – Dataset::current() would be wrong for mixed-depth layer stacks
            if (Dataset::datasets[layerId].bytesPerVoxel == 2) {
                dcSliceExtract(reinterpret_cast<std::uint16_t *>(datacube), &currentPxInDc_float, texData.data(), s, &t, vp.v2, layerId, adjustment, vp.texture.usedSizeInCubePixels);
            } else {
                dcSliceExtract(reinterpret_cast<std::uint8_t *>(datacube), &currentPxInDc_float, texData.data(), s, &t, vp.v2, layerId, adjustment, vp.texture.usedSizeInCubePixels);
            }
