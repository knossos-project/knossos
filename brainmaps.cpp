...
void bmundo() {
    ...
    abortMeshDownload = false;
    for (auto rIt = std::rbegin(Skeletonizer::singleton().skeletonState.trees); rIt != std::rend(Skeletonizer::singleton().skeletonState.trees); rIt++) {
        if (rIt->mesh == nullptr) {
...
void splitHightlightToSelection() {
    ...
    std::vector<treeListElement *> treeSelection;
    for (auto treeId : split0ids) {
        treeSelection.emplace_back(Skeletonizer::singleton().findTreeByTreeID(treeId));
    }
    Skeletonizer::singleton().selectTrees(treeSelection);
    split0ids.clear();
}
