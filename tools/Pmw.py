...
def setversion(version):
    if version != _VERSION:
        valErr = ValueError('Dynamic versioning not available')
        raise valErr
...
# MegaToplevel class contains a interface Toplevel widget to act as the
...
# as a Dialog window.  The MegaWidget class contains a interface Frame
...
def forwardmethods(fromClass, toClass, toPart, exclude = ()):
    ...
    if isinstance(toPart, str) == False:
        ...
        # It's not a valid type
            ...
            typeErr = TypeError('toPart must be attribute name, function or method')
            raise typeErr
...
class MegaArchetype:
    ...
    def __init__(self, parent = None, hullClass = None):
        ...
        if hullClass is None:
            ...
            if parent is None:
                parent = interface._default_root
    ...
    def createcomponent(self, componentName, componentAliases,
        ...
        valErr = ValueError("Component name '" + componentName \
                            + "' must not contain '_'")
    ...
    def initialiseoptions(self, dummy = None):
        ...
        if self._initialiseoptions_counter == 0:
            ...
            if len(unusedOptions) > 0:
                ...
                keyErr = KeyError(text + string.join(unusedOptions, ', ') + \
        ...
        for option, value in kw.items():
            if optionInfo_has_key(option):
                ...
                if optionInfo[option][FUNCTION] is INITOPT:
                    keyErr = KeyError('Cannot configure initialisation option "' \
                ...
                if index >= 0:
                    ...
                    if componentInfo_has_key(component):
                        ...
                        if len(componentConfigFuncs) == 0 and \
                            ...
                            keyErr = KeyError('Unknown option "' + option + \
                                    '" for ' + self.__class__.__name__)
                            raise KeyErr
...
def popgrab(window):
    ...
    if prevFocus != '':
        ...
        except interface.TclError:
            # Previous focus widget has been deleted. Set focus
            # to root window.
            interface._default_root.focus_set()
    else:
...
def releasegrabs():
    ...
    current = interface._default_root.grab_current()
...
class MegaToplevel(MegaArchetype):
    ...
    def __init__(self, parent = None, **kw):
        ...
        MegaArchetype.__init__(self, parent, interface.Toplevel)
    ...
    def show(self, master = None):
        ...
        if master is not None:
            if master == 'parent':
                ...
                # the current version of interface returns a string.
    ...
    def activate(self, globalMode = 0, geometry = 'centerscreenfirst'):
        if self._active:
            valErr = ValueError('Window is already active')
            raise valErr
        if self.state() == 'normal':
            self.withdraw()
...
forwardmethods(MegaToplevel, interface.Toplevel, '_hull')
...
class MegaWidget(MegaArchetype):
    def __init__(self, parent = None, **kw):
        ...
        # Initialise the base class (after defining the options).
        MegaArchetype.__init__(self, parent, interface.Frame)

        # Check keywords and initialise options.
        self.initialiseoptions()

forwardmethods(MegaWidget, interface.Frame, '_hull')
...
def showbusycursor():
    ...
    for (window, winInfo) in _toplevelBusyInfo.items():
        if (window.state() != 'withdrawn' and not winInfo['isBusy']
            ...
            # through to interface, otherwise it will crash in
...
def setbusycursorattributes(window, **kw):
    ...
    for name, value in kw.items():
        if name == 'exclude':
            ...
            keyErr = KeyError('Unknown busycursor attribute "' + name + '"')
            raise KeyErr

def _addRootToToplevelBusyInfo():
    # Include the Tk root window in the list of toplevels.  This must
    # not be called before interface has had a chance to be initialised by
    # the application.
...
def busycallback(command, updateFunction = None):
    if not callable(command):
        valErr = ValueError('cannot register non-command busy callback %s %s' % \
...
def initialise(
    ...
    # create one.
    if root is None:
        if interface._default_root is None:
            root = interface.Tk()
        else:
            root = interface._default_root
    ...
    # Trap interface Toplevel constructors so that a list of Toplevels
    # can be maintained.
    interface.Toplevel.title = __interfaceToplevelTitle

    # Trap interface widget destruction so that megawidgets can be
    # destroyed when their hull widget is destoyed and the list of
    # Toplevels can be pruned.
    interface.Toplevel.destroy = __interfaceToplevelDestroy
    interface.Widget.destroy = __interfaceWidgetDestroy

    # Modify interface's CallWrapper class to improve the display of
    # errors which occur in callbacks.
    interface.CallWrapper = __interfaceCallWrapper
...
class _TraceTk:
    ...
    def call(self, *args, **kw):
        ...
        try:
        ...
        except interface.TclError as errorString:
            _callToTkReturned = 1
            _recursionCounter = _recursionCounter - 1
            _traceTkFile.write('\nTK ERROR> %d:%s-> %s\n' %
            ...
            TclErr = interface.TclError(errorString)
            raise TclErr
...
def _setinterfaceps(window, tk):
    window.tk = tk
    for child in window.children.values():
      _setinterfaceps(child, tk)
...
# called from the interface Toplevel constructor.
...
def __interfaceToplevelTitle(self, *args):
    ...
    return apply(interface.Wm.title, (self,) + args)
...
# Modify the interface destroy methods so that it notifies us when a Tk
...
def __interfaceToplevelDestroy(tkWidget):
    if _hullToMegaWidget.has_key(tkWidget):
        ...
        interface.BaseWidget.destroy(tkWidget)

def __interfaceWidgetDestroy(tkWidget):
    if _hullToMegaWidget.has_key(tkWidget):
        mega = _hullToMegaWidget[tkWidget]
        try:
            mega.destroy()
...
# Add code to interface to improve the display of errors which occur in
# callbacks.

class __interfaceCallWrapper:
...
class _ErrorWindow:
    def __init__(self):
        ...
        upperframe = interface.Frame(self._top)

        scrollbar = interface.Scrollbar(upperframe, orient='vertical')
        scrollbar.pack(side = 'right', fill = 'y')

        self._text = interface.Text(upperframe, yscrollcommand=scrollbar.set)
        self._text.pack(fill = 'both', expand = 1)
        scrollbar.configure(command=self._text.yview)

        # Create the buttons and label in a frame
        lowerframe = interface.Frame(self._top)

        ignore = interface.Button(lowerframe,
                text = 'Ignore remaining errors', command = self._hide)
        ignore.pack(side='left')

        self._nextError = interface.Button(lowerframe,
                text = 'Show next error', command = self._next)
        self._nextError.pack(side='left')

        self._label = interface.Label(lowerframe, relief='ridge')
    ...
    def __init__(self, parent = None, **kw):
        ...
        if width > 0:
            self._separator = self.createcomponent('separator',
                    (), None,
                    interface.Frame, (oldInterior,), relief = 'sunken',
...
def timestringtoseconds(text, separator = ':'):
  ...
  if len(inputList) != 3:
    valErr = ValueError('invalid value: ' + text)
    raise valErr
...
class Balloon(Pmw.MegaToplevel):
    ...
    def unbind(self, widget):
        if hasattr(widget, '_Pmw_BalloonBindIds'):
            if widget._Pmw_BalloonBindIds.has_key(None):
                (enterId, motionId, leaveId, buttonId, destroyId) = \
                        widget._Pmw_BalloonBindIds[None]
                # Need to pass in old bindings, so that interface can
    ...
    def tagbind(self, widget, tagOrItem, balloonHelp, statusHelp = None):
        ...
        motionId = widget.tag_bind(tagOrItem, '<Motion>', 
                lambda event = None, self = self, statusHelp = statusHelp:
                        self.showstatus(statusHelp))
        leaveId = widget.tag_bind(tagOrItem, '<eave>', self._leave)
    ...
    def _state(self):
        if self['state'] not in ('both', 'balloon', 'status', 'none'):
            valErr = ValueError('bad state option ' + repr(self['state']) + \
    ...
    def _relmouse(self):
        if self['relmouse'] not in ('both', 'x', 'y', 'none'):
            valErr = ValueError('bad relmouse option ' + repr(self['relmouse'])+ \
    ...
    def _destroy(self, event):
        ...
        # Note that in a interface Destroy event, the widget field is a
...
class ButtonBox(Pmw.MegaWidget):
    def __init__(self, parent = None, **kw):
        ...
        if orient == 'horizontal':
            ...
            valErr = ValueError('bad orient option ' + repr(orient) + \
    ...
    def index(self, index, forInsert = 0):
        ...
        if type(index) == types.IntType:
            if forInsert and index <= listLength:
                ...
                valErr = ValueError('index "%s" is out of range' % index)
                raise valErr 
    ...
    def insert(self, componentName, beforeComponent = 0, **kw):
        if componentName in self.components():
            valErr = ValueError('button "%s" already exists' % componentName)
            raise valErr
        ...
        button = apply(self.createcomponent, (componentName,
                (), 'Button',
                interface.Button, (self._buttonBoxFrame,)), kw)
...
class EntryField(Pmw.MegaWidget):
    ...
    def __init__(self, parent = None, **kw):
        ...
        # Also create bindings if the interface default interpreter has
        # changed.  Use interface._default_root to create class
        ...
        if EntryField._classBindingsDefinedFor != interface._default_root:
            ...
            for tag in tagList:
                ...
                if type(sequences) is types.StringType:
                    # In old versions of interface, bind_class returns a string
    ...
    def _checkValidateFunction(self, function, option, validator):
        ...
        if function is not None and not callable(function):
            ...
            valErr = ValueError(msg % (option, validator, standard, extra))
            raise valErr
...
Pmw.forwardmethods(EntryField, interface.Entry, '_entryFieldEntry')
...
class MainMenuBar(Pmw.MegaArchetype):
    ...
    def __init__(self, parent = None, **kw):
        ...
        # Initialise the base class (after defining the options).
        Pmw.MegaArchetype.__init__(self, parent, interface.Menu)
    ...
    def _addmenu(self, parentMenuName, menuName, balloonHelp, statusHelp,
        ...
        if (menuName) in self.components():
            valErr = ValueError('menu "%s" already exists' % menuName)
            raise valErr 
        ...
        menu = apply(self.createcomponent, (menuName,
                (), 'Menu',
                interface.Menu, (parentMenu,)), menukw)
    ...
    def addmenuitem(self, menuName, itemType, statusHelp = '',
        ...
        if itemType == 'command':
            ...
            valErr = ValueError('unknown menuitem type "%s"' % itemType)
            raise valErr
...
Pmw.forwardmethods(MainMenuBar, interface.Menu, '_hull')
    ...
    def _addmenu(self, parentMenuName, menuName, balloonHelp, statusHelp,
        ...
        if parentMenuName is None:
            button = apply(self.createcomponent, (menuName + '-button',
                    (), 'Button',
                    interface.Menubutton, (self.interior(),)), kw)
...
Pmw.forwardmethods(MessageBar, interface.Entry, '_messageBarEntry')
...
class NoteBook(Pmw.MegaArchetype):
    ...
    def __init__(self, parent = None, **kw):
        ...
        # Initialise the base class (after defining the options).
        Pmw.MegaArchetype.__init__(self, parent, interface.Canvas)
        ...
        #   page           the interface.Frame widget for the page
        #   created        set to true the first time the page is raised
        #   tabbutton      the interface.Button widget for the button (if any)
    ...
    def insert(self, pageName, before = 0, **kw):
        ...
        if self._withTabs:
            ...
            tab = apply(self.createcomponent, (pageName + '-tab',
                    (), 'Tab',
                    interface.Button, self._hull), tabOptions)
    ...
    def index(self, index, forInsert = 0):
        ...
        if type(index) == types.IntType:
            ...
            if forInsert:
                ...
                valErr = ValueError('NoteBook has no pages')
                raise valErr
...
# forwarded.
Pmw.forwardmethods(NoteBook, interface.Canvas, '_hull')
...
class OptionMenu(Pmw.MegaWidget):
    ...
    def __init__(self, parent = None, **kw):
        ...
        self._menu = self.createcomponent('menu',
                (), None,
                interface.Menu, (self._menubutton,),
                tearoff=0
        )
    ...
    def index(self, index):
        ...
        if type(index) == types.IntType:
            ...
            if listLength > 0:
                ...
                valErr = ValueError('OptionMenu has no items')
                raise valErr
        else:
...
class PanedWidget(Pmw.MegaWidget):
    ...
    def __init__(self, parent = None, **kw):
        ...
        if self['orient'] not in ('horizontal', 'vertical'):
            valErr = ValueError('bad orient option ' + repr(self['orient']) + \
                ': must be either \'horizontal\' or \'vertical\'')
            raise valErr
    ...
    def _parsePaneOptions(self, name, args):
        ...
        for arg, value in args.items():
            ...
            if arg == 'size':
                ...
                valErr = ValueError('keyword must be "size", "min", or "max"')
                raise valErr
    ...
    def _getNaturalSizes(self):
        ...
        if self['orient'] == 'vertical':
            ...
            majorspec = interface.Frame.winfo_reqheight
        else:
            self._majorSize = self.winfo_width()
            self._minorSize = self.winfo_height()
            majorspec = interface.Frame.winfo_reqwidth
...
class RadioSelect(Pmw.MegaWidget):
    ...
    def __init__(self, parent = None, **kw):
        ...
        if self['selectmode'] == 'single':
            ...
            valErr = ValueError('bad selectmode option "' + \
                    self['selectmode'] + '": should be single or multiple')
            raise valErr

        if self['buttontype'] == 'button':
            self.buttonClass = interface.Button
        elif self['buttontype'] == 'radiobutton':
            self._singleSelect = 1
            self.var = interface.StringVar()
            self.buttonClass = interface.Radiobutton
        elif self['buttontype'] == 'checkbutton':
            self._singleSelect = 0
            self.buttonClass = interface.Checkbutton
        else:
            valErr = ValueError('bad buttontype option "' + \
                    self['buttontype'] + \
    ...
    def index(self, index):
        ...
        if type(index) == types.IntType:
            ...
            if listLength > 0:
                ...
                valErr = ValueError('RadioSelect has no buttons')
                raise valErr
        else:
...
class ScrolledCanvas(Pmw.MegaWidget):
    def __init__(self, parent = None, **kw):
        ...
        if self['borderframe']:
            ...
            self._borderframe = self.createcomponent('borderframe',
                    (), None,
                    interface.Frame, (self.origInterior,),
                    relief = 'sunken',
                    borderwidth = 2,
            )
            ...
            self._canvas = self.createcomponent('canvas',
                    (), None,
                    interface.Canvas, (self._borderframe,),
            ...
            self._canvas = self.createcomponent('canvas',
                    (), None,
                    interface.Canvas, (self.origInterior,),
                    relief = 'sunken',
                    borderwidth = 2,
            )
        ...
        self._horizScrollbar = self.createcomponent('horizscrollbar',
                (), 'Scrollbar',
                interface.Scrollbar, (self.origInterior,),
                orient='horizontal',
    ...
    # (grid_)bbox method inherited from interface.Frame.Grid.
    def bbox(self, *args):
        return apply(self._canvas.bbox, args)

Pmw.forwardmethods(ScrolledCanvas, interface.Canvas, '_canvas')
...
Pmw.forwardmethods(ScrolledField, interface.Entry, '_scrolledFieldEntry')
    ...
    def __init__(self, parent = None, **kw):
        ...
        if self['borderframe']:
            ...
            self._clipper = self.createcomponent('clipper',
                    (), None,
                    interface.Frame, (self._borderframe,),
                    width = 400,
                    height = 300,
                    highlightthickness = 0,
                    borderwidth = 0,
...
class ScrolledListBox(Pmw.MegaWidget):
    ...
    def __init__(self, parent = None, **kw):
        ...
        if ScrolledListBox._classBindingsDefinedFor != interface._default_root:
            root  = interface._default_root
    ...
    def setvalue(self, textOrList):
        ...
        if type(textOrList) == types.StringType:
            if textOrList in listitems:
                ...
                valErr = ValueError('no such item "%s"' % textOrList)
                raise valErr
        else:
            for item in textOrList:
                if item in listitems:
                    ...
                    valErr = ValueError('no such item "%s"' % item)
                    raise valErr
    ...
    # Override interface.Listbox get method, so that if it is called with
    ...
    # Need to explicitly forward this to override the stupid
    # (grid_)size method inherited from interface.Frame.Grid.
    def size(self):
...
Pmw.forwardmethods(ScrolledListBox, interface.Listbox, '_listbox')
...
class ScrolledText(Pmw.MegaWidget):
    def __init__(self, parent = None, **kw):
        ...
        if self['borderframe']:
            ...
            self._textbox = self.createcomponent('text',
                    (), None,
                    interface.Text, (self._borderframe,),
                    highlightthickness = 0,
                    borderwidth = 0,
            )
    ...
    # Override interface.Text get method, so that if it is called with
...
Pmw.forwardmethods(ScrolledText, interface.Text, '_textbox')
...
class SelectionDialog(Pmw.Dialog):
    ...
    # Need to explicitly forward this to override the stupid
    # (grid_)size method inherited from interface.Toplevel.Grid.
    def size(self):
        return self.component('listbox').size()

    # Need to explicitly forward this to override the stupid
    # (grid_)bbox method inherited from interface.Toplevel.Grid.
    def bbox(self, index):
    ...
    def __init__(self, parent = None, **kw):
        ...
        if self['dropdown']:
            ...
            self._arrowBtn = self.createcomponent('arrowbutton',
                    (), None,
                    interface.Canvas, (interior,), borderwidth = 2,
                    relief = 'raised',
                    width = 16, height = 16)
            if 'n' in self['sticky']:
                sticky = 'n'
    ...
    def selectitem(self, index, setentry=1):
        if type(index) == types.StringType:
            ...
            if text in items:
                ...
                indxErr = IndexError('index "%s" not found' % text)
                raise indxErr
        elif setentry:
...
class Counter(Pmw.MegaWidget):
    ...
    def _datatype(self):
        ...
        if _counterCommands.has_key(datatype):
            ...
            validValues.sort()
            valErr = ValueError(('bad datatype value "%s":  must be a' +
...
def logicalfont(name='Helvetica', sizeIncr = 0, **kw):
  if not _fontInfo.has_key(name):
    valErr = ValueError('font %s does not exist' % name)
    raise valErr
