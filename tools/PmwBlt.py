...
def _loadBlt(window):
    if _haveBlt is None:
        if window is None:
            window = interface._default_root
            if window is None:
                window = interface.Tk()
        _checkForBlt(window)
...
class Vector:
    ...
    def __init__(self, size=None, master=None):
        ...
        if master:
            ...
            self._master = interface._default_root
    ...
    # Method borrowed from interface.Var class:
    def get(self):
        return self[:]
...
def _doConfigure(widget, subcommand, option, kw):
    ...
    if option:
        ...
        if kw:
            ...
            valErr = ValueError('cannot have option argument with keywords')
            raise valErr
        option = '-' + option
...
class Graph(interface.Widget):
    # Wrapper for the blt graph widget, version 2.4.

    def __init__(self, master=None, cnf={}, **kw):
        _loadBlt(master)
        interface.Widget.__init__(self, master, _graphCommand, cnf, kw)
...
class Stripchart(Graph):
    ...
    def __init__(self, master=None, cnf={}, **kw):
        _loadBlt(master)
        interface.Widget.__init__(self, master, _chartCommand, cnf, kw)

#=============================================================================
class Tabset(interface.Widget): 

    # Wrapper for the blt TabSet widget, version 2.4.

    def __init__(self, master=None, cnf={}, **kw):
        _loadBlt(master)
        interface.Widget.__init__(self, master, _tabsetCommand, cnf, kw)
