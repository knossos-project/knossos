...
std::unordered_set<uint64_t> bucketFill(const Coordinate & seed, boost::multi_array_ref<uint64_t, 3> cube, const uint64_t objIdToSplit, const uint64_t newSubObjId, const std::unordered_set<uint64_t> & subObjectsToFill) {
...
std::unordered_set<uint64_t> verticalSplittingPlane(const Coordinate & pos, boost::multi_array_ref<uint64_t, 3> cube, const uint64_t objIdToSplit, const uint64_t newSubObjId) {
    ...
    while (!work.empty()) {
        ...
        if (subobjectId != 0 && subobjectId != newSubObjId) {
            ...
            if (objId == objIdToSplit) {
                ...
                auto walk = [&](const Coordinate & newPos){
                    if (visitedVoxels.find(newPos) == std::end(visitedVoxels)) {
                        if (pos.z < cube.size() && pos.y < cube[0].size() && pos.x < cube[0][0].size()) {
                            work.emplace_back(newPos);
                        }
                    }
                };
...
void verticalSplittingPlane(const Coordinate & seed) {
    ...
    if (rawcube != HT_FAILURE) {
        ...
        if (subobjectId != 0) {
            ...
            std::unordered_set<uint64_t> subObjectsToFill = verticalSplittingPlane({pos.x, pos.y, pos.z}, cube, splitId, newSubObjId);

            std::unordered_set<uint64_t> visitedSubObjects = bucketFill({pos.x + 1, pos.y, pos.z}, cube, splitId, newSubObjId, subObjectsToFill);
