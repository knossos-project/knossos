...
/* new version, prefix q is only for the transistion phase, since function cannot be overloaded in plain c  */
void qhandleMouseButtonLeft(QMouseEvent *event, int32_t VPFound);
void qhandleMouseButtonMiddle(QMouseEvent *event, int32_t VPFound);
void qhandleMouseButtonRight(QMouseEvent *event, int32_t VPFound);
void qhandleMouseMotion(QMouseEvent *event, int32_t VPFound);
void qhandleMouseMotionLeftHold(QMouseEvent *event, int32_t VPFound);
void qhandleMouseMotionMiddleHold(QMouseEvent *event, int32_t VPFound);
void qhandleMouseMotionRightHold(QMouseEvent *event, int32_t VPFound);
void qhandleMouseWheelForward(QWheelEvent *event, int32_t VPFound);
void qhandleMouseWheelBackward(QWheelEvent *event, int32_t VPFound);
void qhandleKeyboard(QKeyEvent *event);
static Coordinate * qgetCoordinateFromOrthogonalClick(QMouseEvent *event, int32_t VPFound);
