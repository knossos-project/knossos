...
public:
    ...
    void loadMesh(QIODevice &, const boost::optional<decltype(treeListElement::treeID)> treeID);
    void saveMesh(QIODevice & file, const treeListElement & tree);
    void addMeshToTree(boost::optional<decltype(treeListElement::treeID)> treeID, QVector<float> & verts, QVector<float> & normals, QVector<unsigned int> & indices, QVector<float> color, int draw_mode = 0, bool swap_xy = false);
