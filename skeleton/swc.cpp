...
void parseSWC(QIODevice && file) {
    ...
    const auto node = [treeID](auto & ctx){
        ...
        const auto pos = floatCoordinate(at_c<1>(attr), at_c<2>(attr), at_c<3>(attr)) * 1000.0 / Dataset::current().scale;
        const auto radius = at_c<4>(attr) * 1000.0 / Dataset::current().scale.x;
        auto node = Skeletonizer::singleton().addNode(boost::none, radius, treeID, pos, VIEWPORT_UNDEFINED, 1, boost::none, false);
        if (const auto connectedNode = Skeletonizer::singleton().findNodeByNodeID(at_c<5>(attr))) {
