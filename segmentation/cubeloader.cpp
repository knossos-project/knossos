...
bool writeVoxel(const Coordinate & pos, const uint64_t value, bool isMarkChanged) {
    ...
    const auto inCube = pos.insideCube(segLayer.cubeShape, segLayer.scaleFactor);
...
auto wholeCubes = [](const Coordinate & globalFirst, const Coordinate & globalLast, const uint64_t value, CubeCoordSet & cubeChangeSet) {
    const auto segLayer = Dataset::datasets[Segmentation::singleton().layerId];
    const auto wholeCubeBegin = segLayer.global2cube(globalFirst + segLayer.cubeShape - 1);
...
CubeCoordSet processRegion(const Coordinate & globalFirst, const Coordinate &  globalLast, Func func, Skip skip) {
    ...
    for (int x = cubeBegin.x; x < cubeEnd.x; ++x) {
        ...
        if (rawcube.first) {
            auto cubeRef = getCubeRef(rawcube.second);
            const auto globalCubeEnd = globalCubeBegin + segLayer.scaleFactor.componentMul(cubeShape);
            const auto localStart = globalFirst.capped(globalCubeBegin, globalCubeEnd).insideCube(cubeShape, segLayer.scaleFactor);
            const auto localEnd = globalLast.capped(globalCubeBegin, globalCubeEnd).insideCube(cubeShape, segLayer.scaleFactor);
