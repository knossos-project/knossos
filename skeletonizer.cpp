...
bool Skeletonizer::loadXmlSkeleton(QString fileName) {    
    ...
    if(state->skeletonState->mergeOnLoadFlag == false) {
        merge = false;
        clearAnnotation(CHANGE_MANUAL, true);
    }
    else {
        merge = true;
...
bool Skeletonizer::delTree(int targetRevision, int treeID, int serialize) {
    ...
    // delete patches
    while(currentTree->firstPatch != NULL) {
...
bool Skeletonizer::clearAnnotation(int targetRevision, int loadingSkeleton) {
    // This is a SYNCHRONIZABLE skeleton function. Be a bit careful.
    ...
    if(targetRevision == CHANGE_MANUAL) {
        if(!Client::syncMessage("br", KIKI_clearAnnotation)) {
            Client::skeletonSyncBroken();
        }
    }
    else {
...
bool Skeletonizer::addPatchListElement(int patchID) {
    ...
    if(tree == NULL) { // create new active tree, to add patch to
        color4F c; c.r = -1;
        tree = addTreeListElement(true, CHANGE_MANUAL, 0, c, true);
        if(tree == NULL) {
            qDebug("failed to create a new tree for this patch.");
            return false;
        }
        state->skeletonState->workMode = SKELETONIZER_ON_CLICK_ADD_NODE;
        setActiveTreeByID(tree->treeID);
        updateToolsSignal();
    }

    // create new patch object
    Patch *newPatch = Patch::newPatch(patchID);

    // add patchListElement to tree
    patchListElement *newPatchEl = (patchListElement *)malloc(sizeof(patchListElement));
    newPatchEl->patchID = newPatch->patchID;
    newPatchEl->correspondingTree = tree;
    if(tree->firstPatch == NULL) {
        tree->firstPatch = newPatchEl;
        tree->firstPatch->next = tree->firstPatch->previous = tree->firstPatch;
    }
    else {
        tree->firstPatch->previous->next = newPatchEl;
        newPatchEl->previous = tree->firstPatch->previous;
        tree->firstPatch->previous = newPatchEl;
        newPatchEl->next = tree->firstPatch;
    }
    return true;
}

patchListElement *Skeletonizer::findPatchByPatchID(uint patchID) {
    patchListElement *patch;
    treeListElement *currentTree;

    currentTree = state->skeletonState->firstTree;
    while(currentTree) {
        patch = currentTree->firstPatch;
        if(patch == NULL) {
            currentTree = currentTree->next;
            continue;
        }
        do {
            ...
            patch = patch->next;
        } while(patch != currentTree->firstPatch);
...
bool Skeletonizer::delActivePatch() {
    if(Patch::activePatch == NULL) {
        return false;
    }
    //patchEl = findPatchByPatchID(Patch::activePatch->patchID);
    return delPatch(NULL, Patch::activePatch->patchID);
}

bool Skeletonizer::delPatch(patchListElement *patchElToDel, uint patchID) {
    if(patchElToDel == NULL && patchID == -1) {
        return false;
    }
    if(patchElToDel == NULL) {
        patchElToDel = findPatchByPatchID(patchID);
        if(patchElToDel == NULL) {
            qDebug("No patch with ID %i found.", patchID);
            return false;
        }
    }
    // delete patch object
    ...
    if(patchElToDel == patchElToDel->correspondingTree->firstPatch) {
        if(patchElToDel->next == patchElToDel) { // last patch of this tree
            patchElToDel->correspondingTree->firstPatch = NULL;
        }
        else {
            patchElToDel->correspondingTree->firstPatch = patchElToDel->next;
        }
    }
    delete patchElToDel;
    return true;
