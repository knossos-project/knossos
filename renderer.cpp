...
uint Renderer::renderViewportBorders(uint currentVP) {
    ...
    glBegin(GL_LINES);
        ...
        glVertex3d(vp.edgeLength - 1, vp.edgeLength - 1, -1);
        glVertex3d(1, vp.edgeLength - 1, -1);
        glVertex3d(1, vp.edgeLength - 1, -1);
        glVertex3d(1, 1, -1);
    glEnd();
    ...
    if(state->viewerState->vpConfigs[currentVP].type == state->viewerState->highlightVp) {
        ...
        glBegin(GL_LINE_LOOP);
            glVertex3d(3, 3, -1);
            glVertex3d(vp.edgeLength - 3, 3, -1);
            //glVertex3d(vp.edgeLength - 3, 3, -1);
            glVertex3d(vp.edgeLength - 3, vp.edgeLength - 3, -1);
            //glVertex3d(vp.edgeLength - 3, vp.edgeLength - 4, -1);
...
//    std::vector<floatCoordinate> points = Patch::activePatch->pointCloudAsVector(viewportType);
//    glColor4f(0, 1., 0, 1.);
//    glTranslatef(-(float)state->boundary.x / 2. + 0.5,-(float)state->boundary.y / 2. + 0.5,-(float)state->boundary.z / 2. + 0.5);
//    glDeleteBuffers(1, &Patch::vbo);
//    glGenBuffers(1, &Patch::vbo);
//    glBindBuffer(GL_ARRAY_BUFFER, Patch::vbo);
//    glBufferData(GL_ARRAY_BUFFER, points.size()*sizeof(floatCoordinate),
//                 &points[0],
//                 GL_STATIC_DRAW);
...
//        pcl_Mesh::Ptr mesh = Patch::concaveHull(cloud);
//        if(mesh) {
//            pcl_Cloud meshCloud;
...
//            glGenBuffers(1, &Patch::vbo);
//            glBindBuffer(GL_ARRAY_BUFFER, Patch::vbo);
//            glBufferData(GL_ARRAY_BUFFER, meshCloud.points.size()*sizeof(pcl_Point), // size parameter expects size in bytes
//                        &meshCloud.points[0],
//                        GL_STATIC_DRAW);
//            glVertexPointer(3, GL_FLOAT, 0, 0);
//            glEnableClientState(GL_VERTEX_ARRAY);
...
// modified public domain code from: http://www.crownandcutlass.com/features/technicaldetails/frustum.html
bool Renderer::cubeInFrustum(floatCoordinate center, float edgeLen, uint viewportType) {
    for(int p = 0; p < 6; p++) {
       if(state->viewerState->vpConfigs[viewportType].frustum[p][0] * (center.x - edgeLen)
          + state->viewerState->vpConfigs[viewportType].frustum[p][1] * (center.y - edgeLen)
          + state->viewerState->vpConfigs[viewportType].frustum[p][2] * (center.z - edgeLen)
          + state->viewerState->vpConfigs[viewportType].frustum[p][3] > 0) {
          continue;
       }
       if(state->viewerState->vpConfigs[viewportType].frustum[p][0] * (center.x + edgeLen)
          + state->viewerState->vpConfigs[viewportType].frustum[p][1] * (center.y - edgeLen)
...
// modified public domain code from: http://www.crownandcutlass.com/features/technicaldetails/frustum.html
bool Renderer::triangleInFrustum(Triangle tri, uint viewportType) {
    for(int f = 0; f < 6; ++f) {
        if(state->viewerState->vpConfigs[viewportType].frustum[f][0] * tri.a.x
           + state->viewerState->vpConfigs[viewportType].frustum[f][1] * tri.a.y
           + state->viewerState->vpConfigs[viewportType].frustum[f][2] * tri.a.z
           + state->viewerState->vpConfigs[viewportType].frustum[f][3] > 0) {
            continue;
        }
        if(state->viewerState->vpConfigs[viewportType].frustum[f][0] * tri.b.x
           + state->viewerState->vpConfigs[viewportType].frustum[f][1] * tri.b.y
           + state->viewerState->vpConfigs[viewportType].frustum[f][2] * tri.b.z
           + state->viewerState->vpConfigs[viewportType].frustum[f][3] > 0) {
            continue;
        }
        if(state->viewerState->vpConfigs[viewportType].frustum[f][0] * tri.c.x
           + state->viewerState->vpConfigs[viewportType].frustum[f][1] * tri.c.y
           + state->viewerState->vpConfigs[viewportType].frustum[f][2] * tri.c.z
           + state->viewerState->vpConfigs[viewportType].frustum[f][3] > 0) {
           continue;
        }
        // if all corners are behind at least one frustum plane, the triangle is not in the frustum
        return false;
    }
    return true;
}
